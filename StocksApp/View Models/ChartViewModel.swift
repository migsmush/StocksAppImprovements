...
class ChartViewModel: ObservableObject {
    ...
    var foregroundMarkColor: Color {
        (isDragging) ? .cyan : (chart?.lineColor ?? .cyan)
    }
    ...
    private func getDragDateText(timestamp: Date) -> String {
        ...
        return selectedValueDateFormatter.string(from: timestamp)
    }
    ...
    func xAxisChartDataAndItems(_ data: ChartData) -> (ChartAxisData, [ChartViewItem]) {
        ...
        for (index, value) in data.indicators.enumerated() {
            ...
            items.append(ChartViewItem(
                ...
                value: value.close,
                index: index,
                formattedDate: getDragDateText(timestamp: value.timestamp)
            ))
        }
